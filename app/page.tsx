...
import { Droplet, Cpu, ShieldCheck, Zap, MapPin, Phone, Instagram, Menu, X } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen font-sans">
      ...
      {/* Header */}
      <header className="fixed top-0 w-full z-50 transition-all duration-300 border-b border-transparent bg-[#191919]/90 backdrop-blur-md py-4 md:py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <Link href="https://wa.me/553198590098" target="_blank" className="hidden sm:flex btn-primary py-2 px-4 lg:py-2.5 lg:px-6 text-[10px] lg:text-xs uppercase font-display">AGENDAR AGORA</Link>
            <button 
        ...
        {/* Mobile Menu */}
        <div className={`md:hidden absolute top-full left-0 w-full bg-[#191919] border-t border-gray-800 shadow-2xl transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? 'max-h-96' : 'max-h-0 border-t-0'}`}>
          <div className="flex flex-col p-6 gap-6 text-center">
            <Link href="#inicio" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-400 hover:text-white transition-colors uppercase font-display tracking-widest text-sm">Início</Link>
            <Link href="#servicos" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-400 hover:text-white transition-colors uppercase font-display tracking-widest text-sm">Serviços</Link>
            <Link href="#sobre" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-400 hover:text-white transition-colors uppercase font-display tracking-widest text-sm">Sobre</Link>
            <Link href="#contato" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-400 hover:text-white transition-colors uppercase font-display tracking-widest text-sm">Contato</Link>
            <Link href="https://wa.me/553198590098" target="_blank" className="btn-primary py-3 px-6 text-xs uppercase font-display mx-auto w-full max-w-xs mt-2">Agendar Agora</Link>
          </div>
        </div>
      </header>

      {/* Hero Section */}
      <section id="inicio" className="relative min-h-screen flex flex-col justify-end pb-20 md:pb-24 pt-24 md:pt-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10 w-full">
          ...
          <h1 className="font-display text-5xl sm:text-6xl md:text-[7rem] font-bold leading-[1] md:leading-[0.9] tracking-tight mb-6 md:mb-8 uppercase text-white">
            PRECISÃO QUE<br className="hidden md:block"/>MOVE O SEU<br className="hidden md:block"/><span className="text-bosch-red">DESTINO.</span>
          </h1>
          ...
          {/* Call to Action Card */}
          <div className="bg-bosch-red relative overflow-hidden w-full max-w-4xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 md:gap-8 border-l-4 border-white shadow-[0_0_40px_rgba(226,0,21,0.3)] mt-8 group">
            <div className="absolute inset-0 opacity-10 bg-dot-pattern"></div>
            <div className="relative z-10 w-full md:w-auto overflow-hidden">
              <h2 className="font-display text-2xl md:text-4xl font-black text-white leading-[1] md:leading-[0.95] tracking-tight mb-3 uppercase">
                DIREÇÃO DURA OU BARULHO?<br className="hidden md:block"/>RESOLVA AGORA.
              </h2>
              <div className="w-full overflow-hidden whitespace-nowrap flex relative">
                <div className="animate-marquee">
                  <p className="font-display text-red-900 text-xs md:text-sm tracking-[0.2em] font-bold pr-8">
            ...
            <Link href="https://wa.me/553198590098" target="_blank" className="relative z-10 bg-[#111111] text-white font-display uppercase tracking-widest px-8 py-4 text-sm hover:bg-white hover:text-[#111111] transition-colors duration-300 w-full md:w-auto text-center whitespace-nowrap flex-shrink-0">
              WHATSAPP
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-2 md:grid-cols-4">
            {[
              ...
              <div key={idx} className={`py-6 md:py-8 px-2 md:px-4 flex flex-col items-center justify-center text-center border-gray-800 ${idx % 2 !== 0 ? "border-l" : ""} ${idx > 1 ? "border-t md:border-t-0" : ""} ${idx > 0 ? "md:border-l" : ""}`}>
                <span className="font-display text-gray-500 text-[10px] md:text-xs tracking-widest uppercase mb-1">{stat.label}</span>
                <span className="font-display text-lg md:text-2xl tracking-wide text-white uppercase">{stat.value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            ...
            <div className="lg:col-span-7">
              ...
              <h2 className="font-display text-4xl md:text-7xl font-bold leading-[1] md:leading-[0.95] mb-6 md:mb-8 uppercase text-white">
                SOLUÇÕES EM<br className="hidden md:block"/><span className="text-bosch-red">DIREÇÃO.</span>
              </h2>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
            <div className="text-center md:text-left">
              <h2 className="font-display text-3xl md:text-5xl font-bold leading-tight mb-4 uppercase text-white">
                TECNOLOGIA ALEMÃ<br className="hidden md:block"/>AO SEU DISPOR.
              </h2>
            ...
            <div className="bg-white p-6 md:p-8 rounded shadow-xl flex items-center justify-center w-full md:w-auto">
              <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/16/Bosch-logo.svg/1280px-Bosch-logo.svg.png" alt="Bosch" className="h-8 md:h-12" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <h2 className="font-display text-4xl md:text-6xl font-bold leading-[1] md:leading-[0.95] mb-8 md:mb-10 uppercase text-white">
                ESTAMOS EM<br className="hidden md:block"/><span className="text-bosch-red">CONTAGEM.</span>
              </h2>
      ...
      {/* Final CTA */}
      <section className="py-20 md:py-32 bg-bosch-red relative overflow-hidden text-center">
        <div className="absolute inset-0 opacity-10 bg-dot-pattern"></div>
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <h2 className="font-display text-4xl sm:text-5xl md:text-8xl font-black text-white leading-[1] md:leading-[0.9] tracking-tight mb-6 md:mb-8 uppercase">
            SEGURANÇA NÃO É<br className="hidden md:block"/>OPCIONAL. É BOSCH.
          </h2>
